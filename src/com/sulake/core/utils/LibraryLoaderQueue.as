package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1711:int = 4;
      ...
      {
         return this.var_438.length + this.var_306.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_306.length < const_1711 && this.var_438.length > 0)
            {
               _loc1_ = this.var_438.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_438.indexOf(loader);
            if(index > -1)
            {
               this.var_438.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_438)
            {
               if(param2)
               {
