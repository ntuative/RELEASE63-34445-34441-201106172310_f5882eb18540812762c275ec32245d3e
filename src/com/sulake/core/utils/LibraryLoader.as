package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2019:uint = 1;
      
      protected static const const_2021:uint = 2;
      
      protected static const const_2018:uint = 4;
      
      protected static const const_2020:uint = 3;
      ...
      public static const const_1310:Boolean = false;
      ...
      protected var var_262:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_617:Class;
      
      protected var var_502:uint = 0;
      ...
      protected var var_1125:Boolean = false;
      
      protected var var_3002:String = "";
      
      protected var var_3003:String = "";
      
      protected var var_2219:int;
      
      protected var var_1392:int;
      
      protected var var_1391:int;
      
      protected var var_3077:uint = 0;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_45.load(param1.var_262,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1310)
         {
            ...
            while(var_816.length < const_1711 && false)
            {
               _loc1_ = var_1003.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_816.push(_loc1_);
                  _loc1_.var_45.load(_loc1_.var_262,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_262 ? this.var_262.url : null;
      }
      ...
      {
         return !!this.var_1124 ? uint(this.var_1392 - this.var_2219) : uint(getTimer() - this.var_2219);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_423,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_45.close();
            }
      ...
      {
         this.var_262 = param1;
         this._name = parseNameFromUrl(this.var_262.url);
         this.var_1124 = false;
      ...
      {
         if(this.var_1125 && !_disposed)
         {
            this.var_1125 = false;
            if(!this.var_1124 && this.var_262)
            {
               this.var_1392 = -1;
               this.var_2219 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1124 && !_disposed && this.var_1391 > 0)
         {
            try
            {
               this.var_45.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_262,const_1949 - this.var_1391);
            if(this.var_3004)
            {
               this.var_262.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_262.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_3004 = false;
            }
            --this.var_1391;
            this.var_502 = 0;
            this.var_45.load(this.var_262,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_423 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_423 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_423,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_423,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_423))
               {
                  this.var_1392 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_502 = LibraryLoader.name_5;
         ...
         if(this.var_502 == LibraryLoader.const_2020)
         {
         ...
         if(this.var_502 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_423,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         if(this.var_617 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_262.url + "!");
            }
            return false;
         }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_423,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_423,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
