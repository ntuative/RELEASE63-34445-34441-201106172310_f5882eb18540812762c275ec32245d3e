package com.sulake.core.utils.profiler.tracking
{
   import flash.display.BitmapData;
   
   public class TrackedBitmapData extends BitmapData
   {
      
      public static const const_2160:int = 16777215;
      
      public static const const_1222:int = 8191;
      
      public static const const_1275:int = 8191;
      
      public static const const_2267:int = 1;
      
      public static const const_1325:int = 1;
      
      public static const const_1292:int = 1;
      
      private static var var_562:uint = 0;
      
      private static var var_563:uint = 0;
       
      
      private var _owner:Object;
      
      private var _disposed:Boolean = false;
      
      public function TrackedBitmapData(param1:*, param2:int, param3:int, param4:Boolean = true, param5:uint = 4.294967295E9)
      {
         if(param2 > const_1222)
         {
            param2 = const_1222;
         }
         else if(param2 < const_1325)
         {
            param2 = const_1325;
         }
         if(param3 > const_1275)
         {
            param3 = const_1275;
         }
         else if(param3 < const_1292)
         {
            param3 = const_1292;
         }
         super(param2,param3,param4,param5);
         ++var_562;
         var_563 += param2 * param3 * 4;
         this._owner = param1;
      }
      
      public static function get numInstances() : uint
      {
         return var_562;
      }
      
      public static function get allocatedByteCount() : uint
      {
         return var_563;
      }
      
      override public function dispose() : void
      {
         if(!this._disposed)
         {
            var_563 -= width * height * 4;
            --var_562;
            this._disposed = true;
            this._owner = null;
            super.dispose();
         }
      }
   }
}
