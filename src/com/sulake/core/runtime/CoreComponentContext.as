package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1688:String = "asset-libraries";
      
      private static const const_1689:String = "library";
      
      private static const const_1686:String = "service-libraries";
      
      private static const const_1687:String = "library";
      
      private static const const_1683:String = "component-libraries";
      
      private static const const_1684:String = "library";
      
      private static const const_1685:String = "error_data";
      
      private static const const_1690:String = "error_cat";
      
      private static const const_1691:String = "error_desc";
       
      
      private var var_418:LibraryLoaderQueue;
      
      private var var_898:IEventDispatcher;
      
      private var var_588:uint;
      
      private var var_417:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1339:
               debug("Core; using simple frame update handler");
               this.var_417 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_802:
               debug("Core; using complex frame update handler");
               this.var_417 = this.complexFrameUpdateHandler;
               break;
            case Core.const_794:
               debug("Core; using profiler frame update handler");
               this.var_417 = this.profilerFrameUpdateHandler;
               this.var_230 = new Profiler(this);
               attachComponent(this.var_230,[new IIDProfiler()]);
               var_422.addChild(new ProfilerViewer(this.var_230));
               break;
            case Core.const_576:
               debug("Core; using experimental frame update handler");
               this.var_417 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_417 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_418.length;
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_303[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1685,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1690,String(param3));
         ErrorReportStorage.setParameter(const_1691,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_898 = loadingEventDelegate;
         if(this.var_418 == null)
         {
            this.var_418 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1688)[0];
            if(node != null)
            {
               list = node.child(const_1689);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_201,e);
         }
         try
         {
            node = config.child(const_1686)[0];
            if(node != null)
            {
               list = node.child(const_1687);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_201,e);
         }
         try
         {
            node = config.child(const_1683)[0];
            if(node != null)
            {
               list = node.child(const_1684);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_201,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1189);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_898)
         {
            this.var_898.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_898)
         {
            ...
            this.var_898.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_898)
                  {
                     this.var_898.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_588 - this.var_418.length,this.var_588,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_230 ? 0 : this.var_1660 & 0;
         if(_loc3_ == Core.const_576)
         {
            this.var_303[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_303[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_230 ? 0 : this.var_1660 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_106)
         {
            _loc3_ = this.var_303[_loc5_] as Array;
            if(_loc4_ == Core.const_576)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_417(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_722[priority] = 0;
            receivers = this.var_303[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_422.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_106)
         {
            t = getTimer();
            ...
            {
               if(this.var_722[priority] < priority)
               {
                  ++this.var_722[priority];
                  skip = true;
               }
            }
            if(!skip)
