package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_290:IRoomObjectSprite;
      ...
      private var var_1277:Number = 1.0;
      
      private var var_680:BitmapData;
      
      private var var_840:ColorTransform;
      
      private var var_1571:ColorTransform;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_840 = new ColorTransform();
         this.var_840.alphaMultiplier = 1;
         this.var_1571 = new ColorTransform();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_112 = this.var_1278[param1];
         this.var_1042 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1991 >= 0)
         {
            this.var_290 = this._visualization.getSprite(this.var_1991);
            if(this.var_290 && this.var_290.asset)
            {
               if(this.var_290.width <= 1 || this.var_290.height <= 1)
               {
                  return;
               }
               if(this.var_99 == null)
               {
                  this.var_99 = this.var_290.asset.clone();
                  if(this.var_840.alphaMultiplier != 1)
                  {
                     this.var_680 = new BitmapData(this.var_99.width,this.var_99.height,true,4278190080);
                  }
               }
               this.var_2589 = -this.var_290.offsetX;
               this.var_2590 = -this.var_290.offsetY;
               this.var_290.asset = this.var_99;
            }
            if(this.var_99)
            {
            ...
            if(this.var_680)
            {
               this.var_680.fillRect(this.var_680.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_112.y * this.var_1277;
         }
         return 0;
      }
      ...
      {
         if(!this.var_112 || !this.var_290)
         {
            return;
         }
         if(this.var_99 && this.var_290.asset != this.var_99)
         {
            this.var_290.asset = this.var_99;
         }
         if(this.var_1042)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1042 && this.var_112.hasIgnited)
         {
            this.var_1042 = true;
         }
         _loc8_ = this.var_1176 * this.var_1277;
         this.var_112.update();
         if(this.var_1042)
         {
            if(this.var_112.roomObjectSpriteId >= 0)
            ...
            {
               this.var_99.draw(this.var_680,this.var_1992,this.var_840,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_112.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2589 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1277;
               _loc4_ = this.var_2590 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1277;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1571.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_99.draw(_loc7_,this._translationMatrix,this.var_1571,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2771 = parseInt(param1.@size);
         this.var_1991 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1176 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1277 = this.var_2771 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_840.alphaMultiplier = this._blend;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "TeleportErrorHandler" ? true : false;
               _loc21_ = [];
