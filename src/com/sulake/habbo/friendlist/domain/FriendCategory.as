package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_1114 >= this.getPageCount())
         {
            this.var_1114 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_257.length / const_402);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_1114 * const_402;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_1114 + 1) * const_402,this.var_257.length);
      }
