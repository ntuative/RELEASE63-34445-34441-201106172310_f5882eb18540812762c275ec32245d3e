package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1710:IWindowContainer;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1710 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1140 = this.var_1710.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1140)
         {
            this.var_1140.procedure = this.onButtonClick;
         }
         this.var_1139 = this.var_1710.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1139)
         {
            this.var_1139.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_266 >= this.var_72.length - 1)
         {
            this.var_1139.disable();
         }
         else
         {
            this.var_1139.enable();
         }
         if(this.var_266 <= 0)
         {
            this.var_1140.disable();
         }
         else
         {
            this.var_1140.enable();
         }
      }
      ...
      {
         ++this.var_266;
         this.var_266 = Math.min(this.var_266,this.var_72.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_266;
         this.var_266 = Math.max(this.var_266,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_92.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_72.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_72 || this.var_72.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_72[this.var_266][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_72[this.var_266][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_72[this.var_266][1]).value;
