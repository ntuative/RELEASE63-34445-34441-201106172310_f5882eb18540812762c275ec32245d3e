package com.sulake.habbo.friendbar.view.tabs
{
   ...
   public class FriendRequestTab extends FriendEntityTab
   {
      ...
      public function FriendRequestTab()
      {
         ...
         if(_window)
         {
            _window.findChildByName(const_457).visible = false;
         }
      }
      ...
      {
         var _loc3_:* = null;
         var _loc2_:FriendRequestTab = !!var_1413 ? var_1413 : new FriendRequestTab();
         ...
         if(_loc2_.friend)
         {
            if(_loc2_.friend.figure != param1.figure)
            {
               ...
               _loc3_.bitmap = var_498.getAvatarFaceBitmap(param1.figure);
            }
         }
      ...
      {
         var _loc6_:* = null;
         var _loc1_:IWindowContainer = WINDOWING.buildFromXML(ASSETS.getAssetByName(const_2032).content as XML) as IWindowContainer;
         ...
         var _loc5_:IBubbleWindow = _loc1_.findChildByName(const_457) as IBubbleWindow;
         _loc1_.x = 0;
         _loc1_.y = 0;
         _loc1_.width = name_2;
         ...
         var _loc7_:IRegionWindow = WINDOWING.create("ICON",WindowType.const_365,WindowStyle.const_100,WindowParam.const_31,new Rectangle(0,0,25,25)) as IRegionWindow;
         _loc7_.mouseThreshold = 0;
         var _loc8_:IBitmapWrapperWindow = WINDOWING.create("BITMAP",WindowType.const_433,WindowStyle.const_100,WindowParam.const_33,new Rectangle(0,0,25,25)) as IBitmapWrapperWindow;
      ...
      private function bubbleEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         else if(param1.type == WindowMouseEvent.const_25)
         {
            if(param2.name == const_1022)
            {
               ITextWindow(IWindowContainer(param2).getChildByName(REGION_REJECT_TEXT)).textColor = const_1407;
            }
         }
      }
         ...
         if(!disposed)
         {
            if(friend)
            {
               if(friend.figure == param1.figure)
               {
                  ...
                  if(_loc3_)
                  {
                     _loc4_ = var_498.getAvatarFaceBitmap(param1.figure);
                     if(_loc4_)
                     {
