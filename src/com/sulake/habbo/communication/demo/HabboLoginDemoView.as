package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_515:String = "InitConnection";
      ...
      private const const_2033:String = "fuselogin";
      ...
      {
         this.var_185.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_185 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_185.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_165,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_185.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2033,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_185.habboConfiguration)
            {
               if(this.var_185.habboConfiguration.getKey("use.sso","false") == "TeleportErrorHandler")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_185.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_185.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2033,"/");
         ...
         {
            dispatchEvent(new Event(const_515));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_185.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_158.removeEventListener(SSOTicket.const_429,this.onParsedTicketSuccess);
            this.var_158.removeEventListener(SSOTicket.const_375,this.onParsedTicketFailure);
            this.var_158 = null;
         }
         var _loc2_:String = "";
         if(this.var_185.habboConfiguration != null)
         {
            _loc2_ = this.var_185.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_185.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_158 = new SSOTicket(this.var_185.assets,_loc3_,_loc2_,param1);
         this.var_158.addEventListener(SSOTicket.const_429,this.onParsedTicketSuccess);
         this.var_158.addEventListener(SSOTicket.const_375,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_185.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_185.flashClientUrl = this.var_158.flashClientUrl;
         this.var_185.ssoTicket = this.var_158.ticket;
         this.var_158.removeEventListener(SSOTicket.const_429,this.onParsedTicketSuccess);
         this.var_158.removeEventListener(SSOTicket.const_375,this.onParsedTicketFailure);
         this.var_158 = null;
         dispatchEvent(new Event(const_515));
