package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_683:Rectangle;
      
      private var var_2000:int = 0;
      
      private var var_470:IAvatarRenderManager;
      
      private var var_1129:Boolean;
      
      private var var_2002:BitmapData;
      
      private const const_2328:int = 13421772;
      
      private const const_2329:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_846 = param3;
         this._window = param1;
         this.var_1415 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_2001 = param5;
         if(param3 == null)
         {
            this.var_728 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2000 = Math.max(this.var_2000,_loc6_.colorLayerIndex);
            }
         }
         this.var_470 = this.var_77.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_728 != null && !this.var_2001)
            {
               _loc3_ = this.var_728;
            }
            else
         ...
         if(this.isSelected)
         {
            this.var_1415.color = this.const_2328;
         }
         else
         {
            this.var_1415.color = this.const_2329;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_470.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_470.isFigureReady(_loc1_))
         {
            this.var_470.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_384[_loc6_] + "_" + FigureData.const_165;
               _loc4_ = this.var_470.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_683 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2002)
               {
                  ...
                  this.var_2002 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_683.width,this.var_683.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_683.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_683.y;
               if(this.var_2001 && _loc2_.colorLayerIndex > 0)
               {
