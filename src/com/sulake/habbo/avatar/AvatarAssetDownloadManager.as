package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1056:int = 3;
      
      private var var_473:AssetLoaderStruct;
      
      private var var_1055:Timer;
      ...
      private const const_2330:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_473 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1055 = new Timer(this.const_2330,1);
         this.var_1055.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_473)
         {
            this.var_473.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_473.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_473)
         {
            this.var_473.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_473.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1056 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1298);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1056);
            if(this.var_1298.indexOf("?") > 0)
            {
               _loc2_ = this.var_1298 + "&retry=" + this.var_1056;
            }
            else
            {
               _loc2_ = this.var_1298 + "?retry=" + this.var_1056;
            ...
            this.var_473 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2797,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1297[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1297[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2012)
         {
            this.var_1299.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_540[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_540[_loc3_] = _loc6_;
            }
            this.var_1057[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1057)
         {
            _loc7_ = true;
            _loc4_ = this.var_1057[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_540[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1057[_loc3_];
         }
         while(_loc6_ < this.var_859.length)
         {
            _loc5_ = this.var_859[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_859.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1055.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_858.length > 0 && this.var_859.length < this.const_1711)
         {
            _loc1_ = this.var_858[0];
            _loc1_.startDownloading();
            this.var_859.push(this.var_858.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_858.indexOf(param1) == -1 && this.var_859.indexOf(param1) == -1)
         {
