package com.sulake.habbo.messenger.domain
{
   ...
   public class Conversations
   {
      ...
      private var var_491:IConversationsDeps;
      ...
      public function setSelected(param1:Conversation) : void
      {
         ...
         while(this._startIndex + this.var_491.getTabCount() <= _loc2_)
         {
            ++this._startIndex;
         }
      }
      ...
      public function addConversation(param1:int) : Conversation
      {
         ...
         if(_loc2_)
         {
            _loc3_.addMessage(new Message(Message.const_1011,0,this.var_491.getText("messenger.moderationinfo"),""));
         }
         return _loc3_;
      }
      ...
      public function addMessageAndUpdateView(param1:Message) : void
      {
         ...
         this.var_491.addMsgToView(_loc3_,param1);
         if(_loc2_ != this.var_160.length || _loc4_ != _loc3_.newMessageArrived)
         {
            this.var_491.refresh(false);
         }
      }
      ...
      public function setOnlineStatusAndUpdateView(param1:int, param2:Boolean) : void
      {
         ...
         this.var_491.addMsgToView(_loc3_,_loc4_);
      }
      ...
      private function addConversationInt(param1:int) : Conversation
      {
         ...
         _loc2_ = this.var_491.createConversation(param1);
         if(_loc2_ == null)
         {
            return null;
         }
      ...
      {
         this._startIndex = Math.min(this._startIndex,this.var_160.length - this.var_491.getTabCount());
         this._startIndex = Math.max(0,this._startIndex);
      }
      
      private function getOnlineInfoMsg(param1:Boolean) : Message
      {
         return new Message(Message.const_888,0,this.getOnlineText(param1),Util.getFormattedNow());
      }
      
      private function getOnlineText(param1:Boolean) : String
      {
         return this.var_491.getText(!!param1 ? "messenger.notification.online" : "messenger.notification.offline");
      }
   }
}
