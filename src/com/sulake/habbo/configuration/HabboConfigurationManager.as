package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1510:String = "%";
      ...
      public function HabboConfigurationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_720 = this.getKey("url_prefix");
         if(this.var_720 != null && this.var_720.length > 0)
         {
            this.setKey("client.fatal.error.url",this.var_720 + "/flash_client_error");
         }
         this.init();
      }
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. Failed to load URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1179);
      }
      ...
      private function onInitConfiguration(param1:Event = null) : void
      {
         ...
         if(_loc3_ == null || _loc3_.length == 0)
         {
            ...
            Core.error("Could not load external variables, got empty data from URL " + this.getKey("external.variables.txt") + " data length = " + _loc5_ + ". Client startup failed!",false,Core.const_1179);
         }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "TeleportErrorHandler")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "TeleportErrorHandler")
         {
            this.setKey("url.prefix",this.getKey("url.prefix.local"));
         }
         this.var_720 = this.getKey("url.prefix");
         if(this.var_720.length > 0)
         {
            externalAccessDomain = this.var_720.replace("http://","");
      ...
      {
         var _loc2_:String = this.var_1115[param1] as String;
         return _loc2_ != null;
      }
      
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         var _loc4_:String = this.var_1115[param1] as String;
         if(_loc4_ == null)
         {
            _loc4_ = param2;
         }
         if(_loc4_ != null)
         {
            ...
            {
               _loc4_ = _loc4_.replace("%predefined%",this.var_720);
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         if(param4)
         {
            if(this.var_1115[param1] == null)
            {
               Logger.log(param1 + "=" + param2);
            }
         }
         if(this.var_2132.indexOf(param1) < 0 || param3)
         {
            this.var_1115[param1] = param2;
         }
         if(param3)
         {
            this.var_2132.push(param1);
         }
      }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1510);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1510,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      {
         var _loc3_:String = this.var_1115[param1] as String;
         if(_loc3_ == null)
         {
            return param2;
         }
         return _loc3_ == "1" || _loc3_.toLowerCase() == "TeleportErrorHandler";
      }
   }
}
