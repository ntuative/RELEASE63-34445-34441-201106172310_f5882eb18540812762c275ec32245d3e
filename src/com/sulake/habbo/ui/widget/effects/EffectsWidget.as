package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1571:int = 320;
      
      private static const const_1569:int = 48;
      ...
      {
         return var_913 as EffectsWidgetHandler;
      }
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1570;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_387.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_387.add(_loc3_.type,_loc1_);
               this.var_65.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_387.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_387.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_65.removeListItem(_loc1_.window);
               this.var_387.remove(this.var_387.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_65.scrollableRegion.height;
         this.var_65.height = Math.max(Math.min(_loc5_,const_1571),const_1569);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1384,true);
      }
   }
}
