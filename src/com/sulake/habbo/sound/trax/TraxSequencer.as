package com.sulake.habbo.sound.trax
{
   ...
   public class TraxSequencer implements IHabboSound, IDisposable
   {
      ...
      private static const const_195:uint = 8192;
      
      private static const const_1568:uint = 88000;
      
      private static const const_1567:uint = 88000;
      
      private static const const_316:Vector.<int> = new Vector.<int>(const_195,true);
      ...
      private var var_1025:Sound;
      
      private var var_453:SoundChannel;
      
      private var var_1264:TraxData;
      ...
      {
         this.var_223 = uint(param1 * const_117);
      }
      ...
      {
         return this.var_455 / const_117;
      }
      
      public function set fadeOutSeconds(param1:Number) : void
      {
         this.var_455 = int(param1 * const_117);
      }
      
      public function get fadeInSeconds() : Number
      {
         return this.var_827 / const_117;
      }
      
      public function set fadeInSeconds(param1:Number) : void
      {
         this.var_827 = int(param1 * const_117);
      }
      ...
      public function set volume(param1:Number) : void
      {
         ...
         {
            this.var_453.soundTransform = new SoundTransform(this.var_942);
         }
      }
      
      public function get length() : Number
      {
         return this.var_385 / const_117;
      }
      ...
      private function prepareSequence() : Boolean
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.var_1264.channels.length)
         {
            _loc3_ = new Map();
            _loc4_ = this.var_1264.channels[_loc2_] as TraxChannel;
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            while(_loc7_ < _loc4_.itemCount)
            {
               ...
               _loc9_ = this.var_1946.getValue(_loc8_) as TraxSample;
               _loc9_.setUsageFromSong(this.var_1861,_loc1_);
               if(_loc9_ == null)
               {
      ...
      public function play(param1:Number = 0.0) : Boolean
      {
         ...
         this.var_551 = 0;
         this.var_667 = false;
         this.var_1025.addEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         this.var_1555 = param1 * const_117;
         this.var_1263 = 0;
         this.var_1948 = 0;
         this.var_453 = this.var_1025.play();
         this.volume = this.var_942;
         return true;
      }
      ...
      {
         if(this.var_455 > 0 && !this.var_667)
         {
      ...
      private function stopImmediately() : void
      {
         ...
         {
            this.var_1025.removeEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         }
      }
      ...
      private function stopWithFadeout() : void
      {
         if(this.var_668 == null)
         {
            this.var_826 = true;
            this.var_551 = 0;
            this.var_668 = new Timer(this.var_1554 + this.var_455 / (const_117 / 1000),1);
            this.var_668.start();
      ...
      private function mixChannelsIntoBuffer() : void
      {
         ...
         while(_loc4_ >= 0)
         {
            ...
            _loc8_ = const_195;
            if(this.var_385 - this.var_223 < _loc8_)
            {
               _loc8_ = this.var_385 - this.var_223;
            }
            _loc9_ = 0;
            while(_loc9_ < _loc8_)
            {
      ...
      {
         var _loc1_:int = this.var_385 < this.var_1555 ? int(this.var_385) : (this.var_1555 > 0 ? int(this.var_1555) : int(this.var_385));
         if(this.var_223 > _loc1_ + this.var_1554 * (const_117 / 1000) && !this.var_667)
         {
            this.var_667 = true;
            if(this.var_454 != null)
            {
               this.var_454.reset();
               this.var_454.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
            }
            this.var_454 = new Timer(2,1);
            this.var_454.start();
            this.var_454.addEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
         }
         else if(this.var_223 > _loc1_ - this.var_455 && !this.var_826)
         {
            this.var_1027 = false;
            this.var_826 = true;
            this.var_551 = 0;
      ...
      {
         if(param1.position > this.var_1263)
         {
            ++this.var_1948;
            Logger.log("Audio buffer under run...");
            this.var_1263 = param1.position;
         }
         if(this.volume > 0)
         {
            this.mixChannelsIntoBuffer();
         }
         var _loc2_:int = const_195;
         if(this.var_385 - this.var_223 < _loc2_)
         {
            _loc2_ = this.var_385 - this.var_223;
            if(_loc2_ < 0)
            {
               _loc2_ = 0;
            }
         ...
         this.var_223 += const_195;
         this.var_1263 += const_195;
         if(this.var_453 != null)
         {
            this.var_1554 = param1.position / const_117 * 1000 - this.var_453.position;
         }
      ...
      private function writeAudioToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         if(param2 > 0)
         {
            ...
            else
            {
               if(this.var_1027)
               {
                  _loc5_ = 1 / this.var_827;
                  _loc6_ = this.var_1026 / Number(this.var_827);
                  this.var_1026 += const_195;
                  if(this.var_1026 > this.var_827)
                  {
                     this.var_1027 = false;
                  }
               }
               ...
               {
                  _loc5_ = -1 / this.var_455;
                  _loc6_ = 1 - this.var_551 / Number(this.var_455);
                  this.var_551 += const_195;
                  if(this.var_551 > this.var_455)
                  {
                     this.var_551 = this.var_455;
                  }
               }
      ...
      {
         this.stopImmediately();
         this._events.dispatchEvent(new SoundCompleteEvent(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.var_1861));
      }
